...
\end{lemma}
\begin{proof}
Let $P, Q \subseteq \Omega \setminus S$. Let $g(Y) \defeq f(Y \mid X)$. Then
\begin{align*}
& g(P) + g(Q)
\\ &= f(P \cup S) + f(Q \cup S) - 2f(S)
\\ &\ge f((P \cup S) \cup (Q \cup S)) + f((P \cup S) \cap (Q \cap S)) - 2f(S)
...
\end{lemma}
\begin{proof}
\[ s_n = s_k + \sum_{i=k+1}^n a_i \ge s_k + (n-k)a_k \ge s_k + (n-k)\frac{s_k}{k} = \frac{n}{k}s_k.
\qedhere \]
\end{proof}

\begin{lemma}
\label{thm:impl:prop-to-mms}
For any fair division instance $([n], [m], (v_i)_{i=1}^n, w)$,
if $v_i$ is superadditive, then $\WMMS_i \le w_iv_i([m])$ and $\pessShare_i \le w_iv_i([m])$.
\end{lemma}
\begin{proof}
Let $P$ be agent $i$'s WMMS partition. Then
\[ \frac{\WMMS_i}{w_i} = \min_{j=1}^n \frac{v_i(P_j)}{w_j}
    \le \sum_{j=1}^n w_j\left(\frac{v_i(P_j)}{w_j}\right) \le v_i([m]). \]
