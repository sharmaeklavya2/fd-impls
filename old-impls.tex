...
it's now always true that EFX implies EF1.
However, it's true for many common settings, as the following lemma shows.

\begin{lemma}
\label{thm:impl:efx-to-ef1}
In a fair division instance $(N, M, (v_i)_{i \in N}, w)$ of indivisible items,
...
In a fair division instance where entitlements are equal,
the items are goods, and $v_i$ is additive for some agent $i$,
if an allocation $A$ is MXS-fair to agent $i$, then it is also PROP1-fair to agent $i$.
\end{lemma}
