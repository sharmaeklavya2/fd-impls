...
\item $v_i(A_i \setminus S) > w_iv_i(M)$ for every $S \subseteq A_i$
    such that $v_i(S \mid A_i \setminus S) < 0$.
\item For every $j \in N \setminus \{i\}$, define
...
Let $1 \le \ell \le d$. Let $\Pi^d(M)$ be the set of all $d$-partitions of $M$.
Then agent $i$'s $\ell$-out-of-$d$ share is defined as
\[ \textrm{$\ell$-out-of-$d$-share}_i := \max_{\substack{X \in \Pi_d(M):\\v_i(X_j) \le v_i(X_{j+1}) \forall j \in [d-1]}} \sum_{j=1}^{\ell} v_i(X_j). \]
Then agent $i$'s pessimistic share is defined as
\[ \mathrm{pessShare}_i := \sup_{1 \le \ell \le d:\;\ell / d \le w_i} \textrm{$\ell$-out-of-$d$-share}_i. \]
