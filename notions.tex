...
So, we start with the definitions of EFX for additive goods,
and gradually build up to the definition of EFX from there.
...
Let us assume \wLoG{} that $g_3 \in A_2$.
Then agent 1 is not \EFXZero-satisfied, but she is EFX-satisfied.
...
Even $j$'s marginal value over $A_j$ or $A_i$ may be zero.
Before we try to address this issue, let's instead jump to the setting of
additive mixed manna.
