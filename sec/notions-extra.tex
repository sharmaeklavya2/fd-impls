...
doesn't require the good $j$ to have a positive value to $i$ \citep{plaut2020almost}.
...
\begin{lemma}
\label[lemma]{thm:submod-positive-elem}
...
\begin{lemma}
\label[lemma]{thm:submod-negative-elem}
...
However, outside of the identical items case, this nuance doesn't affect our results much.
...
Since marginals are positive, $v_i(S \mid A_i) > 0$ for any $|S| \ge 1$.
Hence, in \cref{defn:propx}, we can assume $|S| = 1$ \wLoG{}.
\end{proof}
...
and is based on \cref{thm:submod-positive-elem,thm:submod-negative-elem}, so we omit the details here.
...
\item Consider 5 items of values $-1000$, $-1000$, $-1000$, $10$, $1$.
    Intuitively, allocation ($\{-1000, 10, 1\}$, $\{-1000\}$, $\{-1000\}$) should not be fair,
    and allocation ($\{-1000, 10\}$, $\{-1000, 1\}$, $\{-1000\}$) should be fair.
