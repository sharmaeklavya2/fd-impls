...
\\[\defaultaddspace] M1S $\fimplies$ PROPx
    & subadditive & $\{0, 1\}$ & -- & $n=2$ & -- & \cref{thm:impl:m1s-to-propx-binary-subadd} & \textbf{new}
\\ \bottomrule
\end{tabular}
...
$w_i < 1/(n-1)$ for some agent $i$, and $v_i$ is subadditive has binary marginals,
...
If $i$ doesn't envy $j$, then $v_i(B_j) \le kw_j/w_i$.
...
Hence, agent $i$'s proportional share is at most $k + w_i(n-1) < k + 1$.
