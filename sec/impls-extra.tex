...
\begin{lemma}[M1S $\fimplies$ PROP1 for binary submodular]
\label{thm:impl:m1s-to-prop1-matroid}
Let $([n], [m], (v_i)_{i=1}^n, \eqEnt)$ be a fair division instance
where $v_i$ is submodular for some agent $i$ and $v_i$ has binary marginals,
i.e., $v_i(g \mid X) \in \{0, 1\}$ for all $X \subseteq [m]$ and $g \in [m] \setminus X$.
If an allocation $A$ is M1S-fair to agent $i$,
then it is also PROP1-fair to agent $i$.
\end{lemma}
\begin{proof}
...
If $\exists g \in [m] \setminus A_i$ such that $v_i(g \mid A_i) = 1$,
then $v_i(A_i \cup \{g\}) = k + 1$, so $A$ is PROP1-fair to $i$.
Now suppose $v_i(g \mid A_i) = 0$ for all $g \in [m] \setminus A_i$. Then
\[ v_i([m]) = v_i(A_i) + v_i([m] \setminus A_i \mid A_i)
    \le k + \sum_{g \in [m] \setminus A_i} v_i(g \mid A_i) = k = v_i(A_i). \]
Hence, $A$ is PROP-fair to $i$.
In all cases, $A$ is PROP1-fair to $i$.
\end{proof}
