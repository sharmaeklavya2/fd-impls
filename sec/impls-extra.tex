...
where $v_i$ is submodular for some agent $i$.
If an allocation $A$ is EF1-fair to agent $i$,
then $A$ is also PROP1-fair to $i$.
\end{lemma}
\begin{proof}
\WLoG, let $i = 1$.
...
Let $g^* \defeq \max_{g \in A_2} v_1(g \mid A_1)$.
If $v_1(g^* \mid A_1) \le 0$, then $v_1(g \mid A_1) \le 0$ for all $g \in A_2$.
...
Hence, $v_1(g^* \mid A_1) > 0$. Therefore,
\begin{align*}
& \frac{v_1(A_1 \cup \{g^*\})}{w_1}
    = v_1(A_1 \cup \{g^*\}) + \frac{w_2}{w_1}v_1(A_1 \cup \{g^*\})
\\ &> v_1(A_1 \cup \{\ghat\}) + \frac{w_2}{w_1}v_1(A_1)
    \ge v_1([m]).
\end{align*}
Hence, $A$ is PROP1-fair to agent 1.
\end{proof}
