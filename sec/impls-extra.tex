...
\begin{algorithm}[htb]
\caption{$\improve(Z)$: Here $Z$ is an allocation for the fair division instance
$([n], G \cup C, (v_i)_{i=1}^n, w)$.}
...
then it is also M1S-fair to agent $i$.
\end{lemma}
\begin{proof}
Let $B$ be agent $i$'s MXS-certificate for $A$.
Then $B$ is EFX-fair to agent $i$ and $v_i(B_i) \le v_i(A_i)$.
Define $\Bhat \defeq \improve(B)$, where $\improve$ is defined in \cref{algo:mxs-to-m1s}.
...
or the allocation gets modified to $Y$, and the following invariants hold:
\begin{tightenum}
\item If $X$ is EFX-fair to $i$, then $Y$ is also EFX-fair to $i$.
\item $|Y_i \cap G| - |Y_i \cap C| > |X_i \cap G| - |X_i \cap C|$.
\item $v_i(Y_i) = v_i(X_i)$.
\end{tightenum}
The first invariant ensures that $\Bhat$ is also EFX-fair to $i$.
The second invariant ensures that $\improve$ terminates.
Then using \cref{thm:special-efx-is-ef1}, we get that $\Bhat$ is EF1-fair to agent $i$.
The third invariant ensures that $v_i(\Bhat_i) = v_i(B_i) \le v_i(A_i)$,
which makes $\Bhat$ an M1S-certificate for allocation $A$ and agent $i$.
The second and third invariants are trivial to show.
We now prove the first invariant.

Suppose $Y_i = X_i \cup \{g\}$ and $Y_j = X_j \setminus \{g\}$,
where $g \in G \cap X_j$ such that $v_i(g \mid X_i) = 0$.
Pick any $S \subseteq Y_j$ such that $v_i(S \mid Y_i) > 0$. Then
\begin{align*}
v_i(S \cup \{g\} \mid X_i) &= v_i(S \cup \{g\} \cup X_i) - v_i(X_i)
\\ &= (v_i(S \cup Y_i) - v_i(Y_i)) + (v_i(Y_i) - v_i(X_i))
\\ &= v_i(S \mid Y_i) > 0.
\end{align*}
Since $X$ is EFX-fair to agent $i$, we get
\[ \frac{v_i(Y_i)}{w_i} = \frac{v_i(X_i)}{w_i}
    \ge \frac{v_i(X_j \setminus (S \cup \{g\}))}{w_j}
    = \frac{v_i(Y_j \setminus S)}{w_j}, \]
so $i$ doesn't EFX-envy agent $j$.
Agent $i$ also doesn't EFX-envy agents in $[n] \setminus \{i, j\}$
since their bundles remain the same. Hence, $i$ is EFX-satisfied by $Y$.

Suppose $Y_i = X_i \setminus \{c\}$ and $Y_j = X_j \cup \{c\}$,
where $c \in X_i \cap C$ such that $v_i(c \mid X_i \setminus \{c\}) = 0$.
Pick any $S \subseteq Y_i$ such that $v_i(S \mid Y_i \setminus S) < 0$. Then
\begin{align*}
v_i(S \cup \{c\} \mid X_i \setminus (S \cup \{c\}))
    &= v(X_i) - v_i(X_i \setminus (S \cup \{c\}))
\\ &= (v(X_i) - v_i(Y_i)) + (v_i(Y_i) - v_i(Y_i \setminus S))
\\ &= v_i(S \mid Y_i \setminus S) < 0.
\end{align*}
Since $X$ is EFX-fair to agent $i$, we get that
for any other agent $k \in [n] \setminus \{i\}$,
\[ \frac{v_i(Y_i \setminus S)}{w_i} = \frac{v_i(X_i \setminus (S \cup \{c\}))}{w_i}
    \ge \frac{v_i(X_k)}{w_k} \ge \frac{v_i(Y_k)}{w_k}. \]
Hence, agent $i$ is EFX-satisfied with $Y$.

Hence, invariant 1 holds, so $\Bhat$ is agent $i$'s M1S-certificate for allocation $A$.
\end{proof}
