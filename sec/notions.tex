...
\begin{tightenum}
\item[(C1)]\label{item:propm:prop}$v_i(A_i) \ge w_iv_i([m])$.
\item[(C2)]\label{item:propm:chores}
    $v_i(A_i \setminus S) > w_iv_i([m])$ for every $S \subseteq A_i$
    such that $v_i(S \mid A_i \setminus S) < 0$.
\item[(C3)]\label{item:propm:goods}
    $T = \emptyset$, or $v_i(A_i) + \max(T) > w_iv_i([m])$.
\item[(C4)]\label{item:propavg}
    $T = \emptyset$, or $v_i(A_i) + \frac{1}{|T|}\sum_{\tau_j \in T}\tau_j > w_iv_i([m])$.
\end{tightenum}
...
The original definitions have minor errors, which our definition fixes.
We explain the errors in \cref{sec:notions:propm,sec:notions:propavg}.
Note that PROPm, PROPavg, and PROPx are equivalent for chores.
