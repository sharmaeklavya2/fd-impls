...
Due to the large number of notions we consider,
we only give a brief mathematical description for each notion.
...
without altering its fundamental properties. See \cref{sec:notions:prop1} for details.
...
When agents have equal entitlements, an allocation is
maximin share (MMS) \cite{budish2011combinatorial} fair to an agent
if her bundle's value is greater than or equal to the maximum she can get by
partitioning the goods into $n$ bundles and picking the worst one.
...
An allocation is MMS-fair to an agent if her bundle's value is at least her maximin share.
See \cref{defn:mms} in \cref{sec:notions:mms} for a more formal definition.
