...
However, a much better relaxation of EF, called \emph{EFX}\fairDefAgain{} (envy-free up to any item),
was proposed in \cite{caragiannis2019unreasonable}.
However, despite considerable effort from researchers, the existence of EFX allocations
...
For the most important setting in fair division---equal entitlements and additive valuations---%
...
We implemented our inference engine as a web application in JavaScript
...
Although some counterexamples we list (\cref{sec:summary}) were already known, most of them are new.
Additionally, we give the simplest possible counterexamples to make
the distinctions between fairness notions as clear and accessible as possible.
This was challenging, especially given the large number of counterexamples we show.
On the other hand, most implications we prove were already known,
but they were known only for additive valuations and equal entitlements,
and we extended them to more general settings.

Some fairness notions were originally defined only for very specific fair division settings,
e.g., EFX was defined in \cite{caragiannis2019unreasonable} only for additive goods.
...
and we present many observations to motivate our definitions.
%\end{enumerate}
...
describe different fair division settings, and introduce associated notation.
...
This paper is an ambitious undertaking, and we have an extensive list of results.
Hence, due to space constraints, we had to move all proofs to the appendices.
The proofs are short and self-contained, so we urge interested reviewers to look at
proofs of a small sample of results.
\fi
