...
\begin{table*}[htb]
\centering
\caption[Non-implications among fairness notions for unit-demand valuations]{%
Non-implications among fairness notions for
identical unit-demand valuations and equal entitlements.
They also hold for cancelable valuations with positive marginals when $m \le 3$,
and submodular valuations with positive marginals.}
\label{table:non-impls-ud}
\begin{tabular}{ccHHccHcH}
\toprule non-implication & valuation & marginals & identical & $n$ & $m$ & entitlements & proof &
\\ \midrule PROP $\nfimplies$ M1S
    & unit-demand & $\ge 0$ & yes & 2 & 3 & equal
    & \cref{cex:prop-not-m1s-unit-demand} & \textbf{new}
\\[\defaultaddspace] PROP1 $\nfimplies$ PROPm
    & unit-demand & $\ge 0$ & yes & 2 & 2 & equal
    & \cref{cex:ud:prop1-not-propm} & \textbf{new}
\\[\defaultaddspace] PROP $\nfimplies$ PPROP
    & unit-demand & $\ge 0$ & yes & 3 & 3 & equal
    & \cref{cex:ud:prop-not-pprop} & \textbf{new}
\\[\defaultaddspace] MMS $\nfimplies$ EF1
    & unit-demand & $\ge 0$ & yes & 4 & 5 & equal
...
\subsection{Submodular and Supermodular Valuations}
\label{sec:cex-extra:submod-supmod}

\begin{lemma}[EF+GAPS $\nfimplies$ PROP1]
\label{cex:ef-not-prop-supmod}
Consider a fair division instance with $n$ agents and $2n$ goods.
The agents have identical valuations and equal entitlements.
Let $a > \eps \ge 0$. The valuation function $v$ is given by
\[ v(X) \defeq \begin{cases}
|X|\eps & \textrm{ if } |X| < 2n
\\ an + |X|\eps & \textrm{ if } |X| = 2n
\end{cases}. \]
Then $v$ is supermodular, and no PROP1 allocation exists.
Let $A$ be the allocation where each agent gets 2 goods.
Then $A$ is EF+GAPS+GMMS, and if $n \ge 3$, then $A$ is also pairwise-PROP.
\end{lemma}
\begin{proof}
For any $X \subseteq [2n]$ and $g \in [2n] \setminus X$, we have
\[ v(g \mid X) = \begin{cases}
\eps & \textrm{ if } |X| < 2n-1
\\ an + \eps & \textrm{ if } |X| = 2n-1
\end{cases}. \]
Since $v(g \mid X)$ is non-decreasing in $|X|$, $v$ is supermodular.

The proportional share is $a+2\eps$.
In any allocation, some agent gets at most 2 goods,
and even if she is given an additional good, her valuation is $3\eps < a+2\eps$.
Hence, no allocation is PROP1.

$v(A_i) = 2\eps$ for all $i \in [n]$, so $A$ is EF.
$A$ is GAPS+GMMS, since after restricting the allocation to any subset of agents,
each agent's MMS is $2\eps$, and if we set the price of each good to 1,
then at most 2 goods are affordable, so the APS is at most $2\eps$.
When $n \ge 3$, then for any $i, j \in [n]$, we have $v(A_i) = v(A_i \cup A_j)/2 = 2\eps$,
so $A$ is pairwise-PROP.
\end{proof}
...
Then $A$ is APS+EFX+PROPx but not EF1 or PROP1.
...
Then $A$ is APS+EF1 but not PROP1.
...
Let $s = (2/5, 3/5, 3/5, 3/5, 3/5)$. Let $0 \le \eps < 1/3$.
Consider a fair division instance with 3 equally-entitled agents
having an identical valuation function $v$ over 5 goods.
$v(X) \defeq \optBP_s(X) + |X|\eps + \eps\boolone(1 \in X)$.
Then $v$ is subadditive by \cref{thm:bin-packing}.

Then allocation $A = (\{1\}, \{2, 3\}, \{4, 5\})$ is EFX, groupwise APS
(set the price of each good to 1), and pairwise PROP.
However, agent 1 is not PROP1-satisfied in $A$, since $v([m]) = 4 + 6\eps$,
and $v(A_1 \cup \{g\}) = 1 + 3\eps$ for all $g \in [5] \setminus A_1$
