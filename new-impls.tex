...
\end{lemma}
\begin{proof}
By \cref{thm:wmms-vs-knappsack}, we get
...
We will construct agent $i$'s epistemic-EF-certificate for $A$ by
fusing an equal number of goods and chores outside her bundle into a single item of value 0
and distributing the remaining items as evenly as possible.

Let $M_+ \defeq \{g \in [m] \setminus A_i: v_i(g) = 1\}$
and $M_- \defeq \{c \in [m] \setminus A_i: v_i(c) = -1\}$.
Pick $\min(|M_+|, |M_-|)$ items each from $M_+$ and $M_-$,
and fuse them together into a single item $t$ of value 0.
Construct an allocation $B$ where $B_i = A_i$,
$t$ is allocated to an arbitrary agent in $[n] \setminus \{i\}$,
and the remaining items (which are either all from $M_+$ or all from $M_-$)
are allocated among $[n] \setminus \{i\}$ by round-robin.
