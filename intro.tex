...
\begin{comment}
A set $M$ of resources (usually represented as a \emph{cake}) must be divided among $n$ agents,
and each agent $i$ has a function $v_i: 2^M \to \mathbb{R}$ assigning a numeric \emph{value} $v_i(S)$
to every subset $S \subseteq M$ of the items.\footnote{%
Formally, $v_i$ is a \emph{measure} over $M$.
See \cite{cohn2013measure} or \cite{stein2009real} for an introduction to measure theory.}
\end{comment}
...
E.g., when 5 identical goods must be divided among two agents,
...
items are not identical and agents have heterogenous preferences,
...
Despite considerable effort from many researchers, the existence of EFX allocations
remains an important open problem.
Hence, relaxations of EFX \cite{caragiannis2023new,amanatidis2020multiple,chaudhury2021little}
and the exisitence of EFX in special cases \cite{chaudhury2024efx,plaut2020almost,amanatidis2021maximum}
has been studied.
%
A similar story has played out for relaxations of PROP:
...
makes the problem even harder, necessitating the use of weaker fairness notions.
...
On the other hand, if we can find an allocation that is $F_1$-fair but not $F_2$-fair,
...
These results help put fairness notions into a hierarchy,
...
\begin{enumerate}
\item EEF implies proportionality (PROP) under subadditive valuations (\cref{thm:impl:mefs-to-prop}).
\item PROP implies MMS for superadditive valuations (\cref{thm:impl:prop-to-wmms}).
\end{enumerate}
We implement our inference engine as a client-side JavaScript web application.
...
For less common settings, like when items are chores or mixed manna,
or when agents have unequal entitlements, much less was known,
so our work makes a significant advancement to understanding the fair division landscape.
...
In \cref{sec:impls,sec:cex}, we present implictions and non-implications, respectively, between fairness notions.
In \cref{sec:feas}, we present results on the feasibility and infeasibility of fairness notions.
